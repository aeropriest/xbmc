...
const char *CPictureDatabase::GetBaseDBName() const
{
    return "MyPicture";
};
...
int CPictureDatabase::openMapDB ()
{

}

void CPictureDatabase::getGeoAddress (double dLatitude, double dLongitude, CStdString &cPlace, CStdString &cCity, CStdString &cCountry)
{
    sqlite3 *db;
    int rc, errormsg;
    char my_qry[500];
    char db_path[500] = "/home/mepl_hy/.xbmc/userdata/Database/mapdata.db";
//    double dlat = 28.961;
//    double dlon = 77.479;
    ...
    try
    {
        ...
        CLog::Log(LOGERROR, "CPictureDatabase::getGeoAddress");
        if( rc )
        {
        ...
        else
        {
            sqlite3_stmt *res;
            const char *tail;
            //sprintf(my_qry, "select (select placedata.place from placedata order by abs(placedata.latitude - %f),  abs(placedata.longitude - %f) asc limit 1) aplace, (select citydata.city from citydata order by abs(citydata.latitude - %f), abs(citydata.longitude - %f) asc limit 1 ) acity, countrydata.country from countrydata where countrydata.city = acity;", dlat, dlon, dlat, dlon);
            sprintf(my_qry, "select (select placedata.place from placedata order by abs(placedata.latitude - %f),  abs(placedata.longitude - %f) asc limit 1) aplace, (select citydata.city from citydata order by abs(citydata.latitude - %f), abs(citydata.longitude - %f) asc limit 1 ) acity, countrydata.country from countrydata where countrydata.city = acity;", dLatitude, dLongitude, dLatitude, dLongitude);

            if( sqlite3_prepare_v2(db, my_qry, strlen(my_qry), &res, &tail) == SQLITE_OK )
            {
                while (sqlite3_step(res) == SQLITE_ROW)
                {
                    cPlace = (char *)sqlite3_column_text(res, 0);
                    cCity = (char *)sqlite3_column_text(res, 1);
                    cCountry = (char *)sqlite3_column_text(res, 2);
                }
            }
            else
            {
    ...
    {
        CLog::Log(LOGERROR, "Picturedatabase:unable to getGeoAddress !!!");
    }
    sqlite3_close(db);
...
void CPictureDatabase::GetFileItemFromDataset(const dbiplus::sql_record* const record, CFileItem* item, const CStdString& strPictureDBbasePath)
{
    ...
    else
    {
        ...
        CStdString strExt = URIUtils::GetExtension(strFileName);
        CStdString path;
        path.Format("%ld%s", record->at(picture_idPicture).get_asInt(), strExt.c_str());
...
bool CPictureDatabase::GetLocationsNav(const CStdString& strBaseDir, CFileItemList& items, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
    try
    {
        ...
        while (!m_pDS->eof())
        {
            ...
            CStdString strDir;
            strDir.Format("%ld/", m_pDS->fv("location.idLocation").get_asInt());
...
bool CPictureDatabase::GetCommonNav(const CStdString &strBaseDir, const CStdString &table, const CStdString &labelField, CFileItemList &items, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
    ...
    try
    {
        ...
        while (!m_pDS->eof())
        {
            ...
            CPictureDbUrl itemUrl = PictureUrl;
            CStdString strDir;
            strDir.Format("%s/", labelValue.c_str());
            itemUrl.AppendPath(strDir);
            pItem->SetPath(itemUrl.ToString());
    ...
    try
    {
        ...
        for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
        {
            ...
            try
            {
                CPictureDbUrl itemUrl = PictureUrl;
                CStdString path;
                path.Format("%ld/", record->at(album_idAlbum).get_asInt());
                itemUrl.AppendPath(path);
